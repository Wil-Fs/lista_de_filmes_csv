...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        movie_data = movie_soup.find('div', attrs={'class': 'ipc-page-content-container ipc-page-content-container--full sc-3c609de6-0 gDoIHY'})
        if movie_data is not None:
            # h1 deve ser o título do nome do filme
            title = movie_data.find('h1', attrs={'class': 'sc-d8941411-0 dxeMrU'}).get_text()
        ...
        plot_text = movie_soup.find('span', attrs={'data-testid': 'plot-l'}).get_text().strip() if movie_soup.find(
